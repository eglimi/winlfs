/*
  ...
  Description: Helper functions for debugging purposes

  Not modified during this thesis.

  CVS Information:
    $Date: 2003/06/30 19:53:40 $
    $Source: /cvsroot/winlfs/code/Ext2Fsd/src/debug.c,v $
    $Revision: 1.4 $
*/
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, FsdGetProcessNameOffset)
#pragma alloc_text(PAGE, Ext2FsdDbgPrintCall)
#pragma alloc_text(PAGE, FsdDbgPrintComplete)
#pragma alloc_text(PAGE, FsdNtStatusToString)
#endif
...
FsdGetProcessNameOffset (VOID)
{
   ...
   {
      if (!strncmp
          (SYSTEM_PROCESS_NAME, (PCHAR) Process + i,
...
VOID
Ext2FsdDbgPrintCall (IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
   ...
   case IRP_MJ_CREATE:
      ...
      if (FileName)
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName);
      }
      ...
      else
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], "Unknown");
      }
   ...
   case IRP_MJ_CLOSE:
      ...
      DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s\n",
                FsdGetCurrentProcessName (), IrpMjStrings[IrpSp->MajorFunction],
                FileName);
   ...
   case IRP_MJ_READ:
      ...
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s IRP_MN_COMPLETE\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName);
      }
      else
      {
         ...
         DbgPrint (DRIVER_NAME
                   ": %-16.16s Offset: %I64u Length: %u Key: %u %s%s%s%s%s%s\n",
                   FsdGetCurrentProcessName (),
                   IrpSp->Parameters.Read.ByteOffset.QuadPart,
                   IrpSp->Parameters.Read.Length, IrpSp->Parameters.Read.Key,
                   (IrpSp->MinorFunction & IRP_MN_DPC ? "IRP_MN_DPC " : ""),
                   (IrpSp->MinorFunction & IRP_MN_MDL ? "IRP_MN_MDL " : ""),
                   (IrpSp->
                    MinorFunction & IRP_MN_COMPRESSED ? "IRP_MN_COMPRESSED " :
                    ""), (Irp->Flags & IRP_PAGING_IO ? "IRP_PAGING_IO " : ""),
                   (Irp->Flags & IRP_NOCACHE ? "IRP_NOCACHE " : ""),
                   (FileObject->
                    Flags & FO_SYNCHRONOUS_IO ? "FO_SYNCHRONOUS_IO " : ""));
      }
   ...
   case IRP_MJ_QUERY_INFORMATION:
      ...
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s %s\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName,
   ...
   case IRP_MJ_DIRECTORY_CONTROL:
...
#else
         FileInformationClass =
            ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.QueryDirectory.
            FileInformationClass;
#endif
...
#else
                  ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.
                  QueryDirectory.FileName
#endif
               )
            {
               DbgPrint (DRIVER_NAME
                         ": %-16.16s FileName: %.*S FileIndex: %#x %s%s%s\n",
                         FsdGetCurrentProcessName (),
#ifndef _GNU_NTIFS_
...
#else
                         ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.
                         QueryDirectory.FileName->Length / 2,
                         ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.
                         QueryDirectory.FileName->Buffer,
                         ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.
                         QueryDirectory.FileIndex,
#endif
                         (IrpSp->
                          Flags & SL_RESTART_SCAN ? "SL_RESTART_SCAN " : ""),
                         (IrpSp->
                          Flags & SL_RETURN_SINGLE_ENTRY ?
                          "SL_RETURN_SINGLE_ENTRY " : ""),
                         (IrpSp->
                          Flags & SL_INDEX_SPECIFIED ? "SL_INDEX_SPECIFIED " :
                          ""));
            }
            else
            {
               DbgPrint (DRIVER_NAME
                         ": %-16.16s FileName: FileIndex: %#x %s%s%s\n",
                         FsdGetCurrentProcessName (),
#ifndef _GNU_NTIFS_
                         IrpSp->Parameters.QueryDirectory.FileIndex,
#else
      ...
      {
         DbgPrint (DRIVER_NAME
                   ": %-16.16s %-31s %s IRP_MN_NOTIFY_CHANGE_DIRECTORY\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName);

         DbgPrint (DRIVER_NAME ": CompletionFilter: %#x %s\n",
#ifndef _GNU_NTIFS_
                   IrpSp->Parameters.NotifyDirectory.CompletionFilter,
#else
                   ((PEXTENDED_IO_STACK_LOCATION) IrpSp)->Parameters.
                   NotifyDirectory.CompletionFilter,
#endif
      ...
      {
         DbgPrint (DRIVER_NAME
                   ": %-16.16s %-31s %s Unknown minor function %#x\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName,
                   IrpSp->MinorFunction);
      }
   ...
   case IRP_MJ_FILE_SYSTEM_CONTROL:
      ...
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s IRP_MN_LOAD_FILE_SYSTEM\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName);
      }
      else
   ...
   case IRP_MJ_DEVICE_CONTROL:
      ...
      DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s IoControlCode: %#x\n",
                FsdGetCurrentProcessName (), IrpMjStrings[IrpSp->MajorFunction],
                FileName, IrpSp->Parameters.DeviceIoControl.IoControlCode);
   ...
   case IRP_MJ_LOCK_CONTROL:
      ...
      {
         DbgPrint (DRIVER_NAME
                   ": %-16.16s %-31s %s IRP_MN_LOCK Offset: %I64u Length: %I64u Key: %u %s%s\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName,
#ifndef _GNU_NTIFS_
...
#endif
                   (IrpSp->
                    Flags & SL_FAIL_IMMEDIATELY ? "SL_FAIL_IMMEDIATELY " : ""),
                   (IrpSp->
                    Flags & SL_EXCLUSIVE_LOCK ? "SL_EXCLUSIVE_LOCK " : ""));
      }
      else if (IrpSp->MinorFunction & IRP_MN_UNLOCK_SINGLE)
      {
         DbgPrint (DRIVER_NAME
                   ": %-16.16s %-31s %s IRP_MN_UNLOCK_SINGLE Offset: %I64u Length: %I64u Key: %u\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName,
#ifndef _GNU_NTIFS_
                   IrpSp->Parameters.LockControl.ByteOffset.QuadPart,
      ...
      {
         DbgPrint (DRIVER_NAME ": %-16.16s %-31s %s IRP_MN_UNLOCK_ALL\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName);
      }
      ...
      {
         DbgPrint (DRIVER_NAME
                   ": %-16.16s %-31s %s IRP_MN_UNLOCK_ALL_BY_KEY Key: %u\n",
                   FsdGetCurrentProcessName (),
                   IrpMjStrings[IrpSp->MajorFunction], FileName,
#ifndef _GNU_NTIFS_
                   IrpSp->Parameters.LockControl.Key
...
VOID
FsdDbgPrintComplete (IN PIRP Irp)
{
   PIO_STACK_LOCATION IrpSp;
   ...
   if (Irp->IoStatus.Status != STATUS_SUCCESS)
   {
      ...
      DbgPrint (DRIVER_NAME ": %-16.16s %-31s *** Status: %s (%#x) ***\n",
                FsdGetCurrentProcessName (), IrpMjStrings[IrpSp->MajorFunction],
...
PUCHAR
FsdNtStatusToString (IN NTSTATUS Status)
{
   switch (Status)
   {
   case 0x00000000:
